...
svmregn = function(data, trainmask, testmask, form_x, form_resp, option) {
  ...
  if (option$probability) {
    if (sum(class(data) == "data.table") > 0) {
      z_values = unique(data[, get(form_resp)])
    } else {
...
binaryExtrapolation_option_forest = list(MuhatFunction = forestregn, options = forestregn_option_bin)
...
bootstrapProps = function(data, form_x, form_z, K, method_prop, option_prop, trim, trim.type, normalize, prop, B, reuse_boot, return_seed, refit_propensities) {
  ...
  boot_data = lapply(1:B, function(b) {
    set.seed(b)
    if (reuse_boot & identical(method_prop, getprop)) {
      ...
      if (refit_propensities == TRUE) {
        #No need to repeat warnings a bunch of times 
        if (b > 3) {
          to_ret = suppressWarnings(cross.fit.propensities(data[realind,], cvboot[realind], form_x, form_z, method_prop, option_prop, trim=trim, trim.type=trim.type, normalize=normalize))
        } else {
          to_ret = cross.fit.propensities(data[realind,], cvboot[realind], form_x, form_z, method_prop, option_prop, trim=trim, trim.type=trim.type, normalize=normalize)
        }
        cvprop = to_ret$prop 
      } else {
  ...
  if (is.null(boot_data[[1]])) {
    stop("boot_data is failing to get bootstrapped propensities")
  } 
  if (return_seed) {
...
dvds = function(Lambdas, data, form_x, form_z, form_y, method_prop, option_prop, 
                method_quant=NULL, option_quant=NULL, method_regn=NULL, option_regn=NULL, 
                method_conddist=NULL, option_conddist=NULL, conddist_quant=F, conddist_kappa=F, 
                K=5, semiadaptive=FALSE, trim=c(0.01,0.99), trim.type='clip', normalize="F", 
                form_x_quant = NULL, form_x_kappa = NULL, stabilize_obj = FALSE, 
                boot_infer = F, boot_settings = list(reuse_boot = T, refit_propensities = F, B = 500, return_seed = TRUE)) {
  ...
  return(foreach(Lambda=Lambdas, .combine=rbind) %do% {
    ...
    for (z in 0:1) {
      for (t in 0:1) {
        ...
        data[['.hinge']] = q + Lambda^(sign((2*t-1) * (data[[form_y]] - q))) * (data[[form_y]] - q)
        if(conddist_kappa) {
        ...
        if (stabilize_obj) {
          ws  = zz * ( (1/ee) / mean( zz / ee ) - 1)
          #Can check unstabilized equivalence with this commended-out block
          #wos = zz * ( (1/ee) / 1 - 1)
          #temp_val = kappa + zz*(q - kappa)/ee + zz*(data[[form_y]] - q)*(ee + (1-ee)*Lambda^((2*t-1)*sign(data[[form_y]] - q)))/ee
          #print(summary(zz * data[[form_y]] + (1-zz) * kappa + wos * (data[['.hinge']] - kappa) - temp_val))
          eif[[paste(z,t)]] = zz * data[[form_y]] + (1-zz) * kappa + ws * (data[['.hinge']] - kappa)
        } else {
        ...
        if (boot_infer) {
          ...
          boot_estims_wrongdirection = mclapply(1:boot_settings$B, function(b) {
            ...
            if (stabilize_obj) {
              ws_star = zz_star * ( (1/e_star) / mean( zz_star / ee_star ) - 1)
              hinge_star = q_star + Lambda^((2*t-1)*sign(data_star[[form_y]] - q_star)) * (Y_star - q_star)
              this_obj = mean(zz_star * Y_star + (1-zz_star) * kappa_star + ws_star * (hinge_star - kappa_star))
            } else {
              this_obj = mean(kappa_star + zz_star*(q_star - kappa_star)/ee_star + zz_star*(data_star[[form_y]] - q_star)*(ee_star + (1-ee_star)*Lambda^((2*t-1)*sign(data_star[[form_y]] - q_star)))/ee_star)
            }
            c(
              this_obj,
              mean(Y_star),
