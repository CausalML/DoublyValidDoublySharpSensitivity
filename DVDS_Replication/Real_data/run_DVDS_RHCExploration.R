...
if (rerun_firststage) {
  ...
  write.csv(all_preds, "Results/Tabs/RHCExploration_predictions.csv", row.names = FALSE)
}
all_preds = read.csv("Results/Tabs/RHCExploration_predictions.csv")
...
extrema.aipw = function(Z, Y, e, mu0, mu1, Lambda=1) {
...
if (rerun_sensitivity) {
  ...
  write.csv(sensitivity_results, "Results/Tabs/sensitivity_results_binary.csv", row.names = FALSE)
  ...
  write.csv(min_lambdas, "Results/Tabs/sensitivity_results_bootstrap_crossing.csv", row.names = FALSE)
} else {
  sensitivity_results = read.csv("Results/Tabs/sensitivity_results_binary.csv")
  min_lambdas         = read.csv("Results/Tabs/sensitivity_results_bootstrap_crossing.csv")
}
...
max_lambda = 1.5
pdf(file = "Results/Figs/RHC_InitialSensitivity_boostOnly_aipwOnly_wide.pdf", width = 6, height = 3)
...
max_lambda = 2
pdf(file = "Results/Figs/RHC_InitialSensitivity_boostOnly_aipwOnly_wide_withCalibration.pdf", width = 6, height = 3.5)
...
fileConn <- file("Results/Tabs/statistics_for_paper.tex")
...
fileConn <- file("Results/Tabs/statistics_sufficient_OR.tex")
