...
# =============================================
# Reading data
binary_dvds = read.table("dvds_output/binary_dvds.csv", header=T) %>% 
  mutate(outcome = "Binary", method = "DVDS")
binary_zsb = read.table("dvds_output/binary_zsb.csv", header=T) %>% 
  mutate(outcome = "Binary", method = "ZSB")
continuous_dvds = read.table("dvds_output/continuous_dvds.csv", header=T) %>%
  mutate(outcome = "Continuous", method = "DVDS")
continuous_zsb = read.table("dvds_output/continuous_zsb.csv", header=T) %>%
...
# =============================================
# Plotting results
pdf(file = "dvds_output/art/binary_comparison.pdf", width=6, height=4)
df %>%
...
pdf(file = "dvds_output/art/continuous_comparison.pdf", width=6, height=4)
df %>%
...
fileConn <- file("dvds_output/binary_coverageRates.tex")
writeLines(coveragePrint(binary.coverage), fileConn)
close(fileConn)


fileConn <- file("dvds_output/continuous_coverageRates.tex")
